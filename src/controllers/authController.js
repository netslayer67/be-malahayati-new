    ...
    try {
        ...
        for (const account of accounts) {
            try {
                ...
                const existingUser = await Admin.findOne({ nama: account.nama });
                if (existingUser) {
                    // Jika sudah ada, lanjutkan ke akun berikutnya
                    console.log(`Pengguna dengan nama ${account.nama} sudah ada dalam database. Lewati...`);
                    continue;
                }
                ...
                // Buat akun baru
                const newAccount = new Admin({ nama: account.nama, password: hashedPassword });
        ...
        if (createdAccounts.length === 0 && errors.length > 0) {
            ...
            return res.status(400).json({ message: 'Tidak ada akun yang berhasil dibuat', errors });
        }

        res.status(201).json({ message: 'Akun berhasil dibuat', accounts: createdAccounts });
    } catch (error) {
        res.status(500).json({ message: 'Terjadi kesalahan saat membuat akun', error: error.message });
    }
};
...
exports.login = async (req, res) => {
    try {
        ...
        if (!isPasswordCorrect) {
            return res.status(400).json({ message: 'Kombinasi nama pengguna dan password salah' });
        }
        ...
        res.status(200).json({ token, id, message: 'AKUN BERHASIL DITEMUKAN' });
    } catch (error) {
